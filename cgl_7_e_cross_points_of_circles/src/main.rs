...
impl Circle {
    ...
    fn cross_points_line(&self, l: &Line) -> (Point, Point) {
    ...
    fn cross_points_circle(&self, other: &Self) -> (Point, Point) {
        let d = Point::distance(self.c, other.c);
        let a = ((self.r * self.r + d * d - other.r * other.r) /
                 (2.0 * self.r * d)).acos();
        let p = other.c - self.c;
        let t = p.y.atan2(p.x);

        (self.c + polar(self.r, t + a), self.c + polar(self.r, t - a))
    }
}    
...
fn main() {
    ...
    let c1 = Circle::new(Point{ x, y }, r);

    let (x, y, r) = scan!(f64, f64, f64);
    let c2 = Circle::new(Point{ x, y }, r);
    
    let (p1, p2) = c1.cross_points_circle(&c2);
